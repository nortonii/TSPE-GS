...
def remove_adjacent_duplicates(batch_imgs: torch.Tensor, similarity_threshold=0.98):
    ...
    plt.cla()
    plt.plot(range(s_sum.shape[0]),s_sum.cpu().numpy())
    plt.ylim(0,1000)
...
def extract_mesh(dataset, pipe, checkpoint_iterations=None):
    ...
    for viewpoint_cam in viewpoint_cam_list:
        ...
        depthes = render_threshold(viewpoint_cam, gaussians, pipe, background, kernel_size,thresholds=np.arange(0.0,0.7,0.7/256))
        print(depthes.shape)
        #cv2.imwrite(depthes)
