...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", is_debug=False, undistorted=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            file_name = frames[idx]["file_path"].split('//')[-1]

            if 'train' in transformsfile:
                cam_name_new = os.path.join(path, "train/" + file_name + extension)
            else:
                cam_name_new = os.path.join(path, "test/" + file_name + extension)

            if not os.path.exists(cam_name) and not os.path.exists(cam_name_new):
                continue
            ...
            if os.path.exists(cam_name_new):
                image_path = os.path.join(path, cam_name_new)
                mask_path = os.path.join(image_path[:-4] + "_alpha" + image_path[-4:])
                image_name = Path(cam_name_new).stem
                image = Image.open(image_path)
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX,
                            image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1], image=image, mask=mask))
