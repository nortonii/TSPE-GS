...
os.environ['CUDA_VISIBLE_DEVICES']=str(2)#np.argmin([int(x.split()[2]) for x in result[:-1]]))
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):       
        ...
        #print((os.path.join(dataset.model_path, "{}_{}.png".format(viewpoint_cam.uid,iteration))))
        if iteration >=20000 and iteration%200==0:
            
            import cv2
            
            cv2.imwrite(os.path.join(dataset.model_path, "{}_{}.png".format(viewpoint_cam.uid,iteration)),
                        (torch.clamp(rendered_image, 0.0, 1.0).permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8))
