...
def find_elbow_k_bic(ks, bics):
    ...
    first_diff=np.zeros(len(bics))
    first_diff[:-1]=np.abs(bics[1:] - bics[:-1])
    first_diff[1:]+=np.abs(bics[1:] - bics[:-1])
    first_diff[1:-1]/=2.0
    if np.max(first_diff)<1500:
        return 1
    #second_diff=np.zeros(len(bics))
    #second_diff[:-1]=np.abs(first_diff[1:] - first_diff[:-1])
    #second_diff[1:]+=np.abs(first_diff[1:] - first_diff[:-1])
    #second_diff[1:-1]/=2.0
    # 对二阶差分取绝对值，找最大肘部点
    elbow_index = np.argmax(np.abs(first_diff))+1  # +2是因为差分减少了两个元素

    if elbow_index >= len(ks):
        elbow_index = len(ks) - 1
...
def remove_adjacent_duplicates(batch_imgs: torch.Tensor, gt_mask):
    #dx = torch.diff(batch_imgs, dim=0)  # 形状 (255, 400, 400)
    #s_sum = dx.abs().sum(dim=(1, 2))
    #s_sum0=1.0/torch.clamp(torch.diff(-batch_imgs.mean(dim=(1,2))),1e-8)
    s_sum1=(batch_imgs).mean(dim=(1,2))
    # 1步差分：长度变成 N-1
...
def extract_mesh(dataset, pipe, checkpoint_iterations=None):
    ...
    for viewpoint_cam in viewpoint_cam_list:
        ...
        depthes = render_threshold(viewpoint_cam, gaussians, pipe, background, kernel_size,thresholds=np.arange(0.0,0.99,0.99/512))
        #cv2.imwrite(depthes)
        ...
        #    cv2.imwrite("depth.png",depth.cpu().numpy()*50)
        #    input()
        depthes=remove_adjacent_duplicates(depthes,viewpoint_cam.gt_mask.squeeze(0))
        #for depth in depthes:
        #    cv2.imwrite("depth.png",depth.cpu().numpy()*50)
        #    input()
        #print(depthes.shape)
