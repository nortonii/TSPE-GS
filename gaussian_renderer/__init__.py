...
import os
def render_adjust(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size,opas = [1.0],scaling_modifier = 1.0, require_coord : bool = True, require_depth : bool = True):
    """
    Render the scene. 
    ...
    depthes=[]
    for multiopa in opas:
        opacity_tmp=opacity*multiopa
        shs = pc.get_features
        colors_precomp = None
        rendered_image, radii, rendered_expected_coord, rendered_median_coord, rendered_expected_depth, rendered_median_depth, rendered_alpha, rendered_normal = rasterizer(
            ...
            opacities = opacity_tmp,
            scales = scales,
            rotations = rotations,
            cov3D_precomp = cov3D_precomp)
        depthes.append(rendered_median_depth)


    # Those Gaussians that were frustum culled or had a radius of 0 were not visible.
    # They will be excluded from value updates used in the splitting criteria.
    return depthes
def render_threshold(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size,thresholds = [1.0],scaling_modifier = 1.0, require_coord : bool = True, require_depth : bool = True):
    """
    Render the scene. 
