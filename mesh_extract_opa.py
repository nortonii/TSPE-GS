...
def remove_adjacent_duplicates(batch_imgs: torch.Tensor, gt_mask):
    ...
    peaks, pin = find_peaks(density,prominence  =0.01*(density.max()-density.min()))  # 找到峰值
    #print(pin)
...
def extract_mesh(dataset, pipe, checkpoint_iterations=None):
    ...
    for viewpoint_cam in tqdm.tqdm(viewpoint_cam_list):
        ...
        depthes = render_threshold(viewpoint_cam, gaussians, pipe, background, kernel_size,thresholds=np.arange(0.02,0.98,0.98/sample_number))
        #cv2.imwrite(depthes)
        ...
        #    cv2.imwrite("depth.png",depth.cpu().numpy()*50)
        #    input()
        depthes,index=remove_adjacent_duplicates(depthes,viewpoint_cam.gt_mask.squeeze(0))
        #for depth in depthes:
        #    cv2.imwrite("depth.png",depth.cpu().numpy()*50)
        #    input()
        #print(depthes.shape)
    ...
    if not single:
        ...
        # 找出哪个类别质心更大
        big_label = np.argmax(centers)

        # 如果大类的标签是0，则翻转标签
        if big_label == 0:
            labels = 1 - labels  # 0变1，1变0

        clustered = {}
        for label, value in zip(labels, merged_list):
    ...
    for depthes, viewpoint_cam in tqdm.tqdm(zip(depth_in, viewpoint_cam_list)):
        for depth in depthes:
            if depth.shape[0]!=0:
                ...
                break
    if flag:
        o3d.io.write_triangle_mesh(os.path.join(dataset.model_path,"recon_all.ply"),mesh.to_legacy())
        print("done")
        exit()
    del vbg
    vbg = o3d.t.geometry.VoxelBlockGrid(attr_names=('tsdf', 'weight'),
    ...
    # 保存或可视化合并后的多边形模型
    o3d.io.write_triangle_mesh(os.path.join(dataset.model_path,"recon_all.ply"),merge_mesh)
    print("done!")
