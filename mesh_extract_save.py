...
def remove_adjacent_duplicates(batch_imgs: torch.Tensor, gt_mask):
    ...
    #plt.cla()
    x_eval = np.linspace(data_np.min(), data_np.max(), 1000)
    density=kde(x_eval)
    #plt.plot(range(1000),density)
    peaks, _ = find_peaks(density,prominence=0.5)  # 找到峰值
...
def extract_mesh(dataset, pipe, checkpoint_iterations=None):
    ...
    for viewpoint_cam in viewpoint_cam_list:
        ...
        #    input()
        #print(depthes.shape)
        depth_list.append(depthes.clone().cpu().unsqueeze(0).half())
        del depthes
    torch.save(torch.cat(depth_list).half(),"dataset/"+os.path.basename(dataset.model_path)+".pt")
    exit()    
