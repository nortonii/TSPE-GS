...
if __name__ == '__main__':
    ...
    if args.gt_path is not None:
        if os.path.isdir(args.gt_path):
            ...
            data_mesh = o3d.io.read_triangle_mesh(args.gt_path)
            stl = sample_mesh(data_mesh, 500000)
        mesh_eval = False


    # 读取预测点云
    if args.input_path.endswith('.ply') or args.input_path.endswith('.pcd'):
        data_pcd = o3d.io.read_point_cloud(args.input_path)
        data_pcd = np.asarray(data_pcd.points)
    elif args.input_path.endswith('.npy'):
        data_pcd = np.load(args.input_path)
    else:
        raise ValueError("Unsupported input_path format for predicted point cloud.")

    # 读取真实点云
    if args.gt_path.endswith('.ply') or args.gt_path.endswith('.pcd'):
        gt_pcd = o3d.io.read_point_cloud(args.gt_path)
        gt = np.asarray(gt_pcd.points)
    elif args.gt_path.endswith('.npy'):
        gt = np.load(args.gt_path)
    else:
        raise ValueError("Unsupported gt_path format for ground truth point cloud.")
